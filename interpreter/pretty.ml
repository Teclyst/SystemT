...
let rec printf_typeT_atom ppf t =
  match t with
  | CoqInterpreter.BoolT -> 
    Format.fprintf ppf "bool"
  | CoqInterpreter.NatT -> 
    Format.fprintf ppf "nat"
  | CoqInterpreter.TvarT x ->
    Format.fprintf ppf "t%a" ident (to_string (nat_to_zt x))
...
and printf_typeT_FunT ppf t =
  ...
  | _ ->
    Format.fprintf ppf "%a" printf_typeT_ProdT t
...
and printf_typeT_ProdT ppf t =
  ...
  | _ ->
    Format.fprintf ppf "%a" printf_typeT_atom t
...
and printf_termT_expr m ppf e =
  ...
  | CoqInterpreter.RecT (e, f, g) ->
      Format.fprintf ppf "rec %a@ %a@ %a" (printf_termT_atom m) e (printf_termT_atom m) f
        (printf_termT_atom m) g
  | CoqInterpreter.IteT (e, f, g) ->
      Format.fprintf ppf "@[<hov> if %a@ then %a@ else %a@]" (printf_termT_expr m) e
        (printf_termT_expr m) f (printf_termT_expr m) g
  | _ -> Format.fprintf ppf "@[<2>%a@]" (printf_termT_appT m) e

and printf_termT_PairT m ppf e =
  match e with
  | CoqInterpreter.PairT (e, f) ->
      Format.fprintf ppf "%a,@ %a" (printf_termT_PairT m) e (printf_termT_expr m) f
  | _ -> printf_termT_expr m ppf e

and printf_termT_absT m ppf e =
  match e with
  | CoqInterpreter.AbsT (CoqInterpreter.AbsT _ as e) ->
      Format.fprintf ppf "_%a@ %a" ident (to_string m) (printf_termT_absT (m + one)) e
...
and printf_termT_appT m ppf e =
  ...
  | CoqInterpreter.AppT (e, f) ->
      Format.fprintf ppf "%a@ %a" (printf_termT_appT m) e (printf_termT_atom m) f
