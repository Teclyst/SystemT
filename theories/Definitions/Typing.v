...
(** Typing derivations.
    ...
    Notice the difference between bound and free variables: as the latter
    are meant to represent already defined constants, we want to be able
    to specialize the type that is given for them within the context.

    Basically, if within G the entry for x is t, with type variables
    t1, ..., tn, then it means that x has type forall t1 ... tn, t (but
    the forall is ommitted, as in classic implementations of the
...
(** Given a type and a list of variables, compute a mapping
    of the variables that occur within the type to fresh
    ones, and the list of those.
*)
    ...
    (TMap.map tvarT s,
      map
        (fun x =>
          ...
          | Some y => y
          | None => TIdent.new nil (* This case does not occur. *)
          end)
        variable_list).

(** Given a term a term, context, and a list of already used type variables,
    builds a unification problem whose solutions (assuming some
    conditions are satisfied by the list) correspond to the typing
    derivations of the term within the context. *)
...
(** Given a term and a mapping of type variables to types,
    finds the most generic type for the term, where every
    occurrence of a free variable must be in the map,
    and then its type must be a specialisation of the entry
    in the map.
*)
